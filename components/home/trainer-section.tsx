        ...
        >
          {trainers.map((trainer: Trainer, index: number) => (
            <Link 
              ...
              onClick={(e) => {
                e.preventDefault();
                setClickedCardId(trainer.id);
                setTimeout(() => {
                  window.location.href = `/${locale}/trainers/${trainer.slug}`;
                }, 700);
              }}
            >
              <Card 
                className={`flex-none bg-card dark:bg-card-dark opacity-0 snap-center w-[220px] min-w-[220px] md:w-[260px] md:min-w-[260px] hover:shadow-lg dark:hover:!scale-[1.02] bg-card dark:bg-card-dark rounded-3xl overflow-hidden cursor-pointer relative group select-none transform-gpu transition-all duration-200 animate-popFadeIn before:content-[''] before:absolute before:top-0 before:right-[-150%] before:w-[150%] before:h-full before:bg-gradient-to-r before:from-transparent before:via-white/25 before:to-transparent before:z-[1] before:pointer-events-none ${clickedCardId === trainer.id ? 'before:animate-shimmer' : ''}`}
                ...
                <CardContent className="p-2 w-full">
                  <div className="flex flex-col w-full text-text-secondary dark:text-text-secondary-dark">
                    <div className="w-full">
                      <h3 className="h3 font-semibold text-lg text-text dark:text-text-dark w-full line-clamp-2 min-h-[2rem]">{trainer.name}</h3>
                      {trainer.sports && trainer.sports.length > 0 && (
                        <div className="w-full flex flex-wrap gap-1 mt-2">
                          {trainer.sports.slice(0, 2).map((sport, idx) => (
                            <span
                              key={idx}
